...
// $ver = "SELECT * FROM minning WHERE Mine_Id = 1003 AND MONTH = 2 AND YEAR = 2564 ORDER BY VERSION LIMIT 1";
$ver = "SELECT * FROM minning WHERE Mine_Id = $mine_id AND Month = $month AND YEAR = $year ORDER BY VERSION DESC LIMIT 1";
$vere = mysqli_query($conn,$ver);
$row = mysqli_fetch_assoc($vere);

$varversion  = $row['Version']+1;

// echo "<script>alert(".$varversion.")</script>";
...
,E_Meterial,E_Cap,E_Ammonia,E_Other,Amount_Employee,Amount_Day,E_N,Version)
            VALUE ('$mine_id','$month','$year','$Wa_New','$Wa_Sum','$Fd_Benzene','$Fd_Diesel','$Fd_Lubricating_Oil','$Fd_Electricity','$Fd_Other'
            ,'$E_Meterial','$E_Cap','$E_Ammonia','$E_Other','$Amount_Employee','$Amount_Day','$E_N','$varversion')";
...
for ($i = 0; $i < count($machine_idArr); $i++) {
    ...
    $sql="INSERT INTO minning_machine (Mine_Id, Month, Year, Machine_Id, Amount,Version)
VALUES ('$mine_id','$month','$year','$machine_idArr[$i]','$Machine_numberArr[$i]','$varversion')";
...
$sql8="INSERT INTO machine_t (Mine_Id, Month, Year, Machine_i, Machine_ot, Machine_hp,no,Version)
VALUES ('$mine_id','$month','$year','$Machine_i' ,'$other_name[$i]','$other_value[$i]','$i','$varversion')";
...
for ($i = 0; $i < count($minerals); $i++) {
    ...
    if(($minerals[$i] != "")){   /* not allowing empty values and the row which has been removed. */
        ...
        // $row2 = mysqli_fetch_assoc($result55);    
            $sql6="INSERT INTO mineral_production (Mine_Id, Month, Year, no, Mp_condition, Mp_Quantity, Mp_Dressing, Mp_sell, Mp_balance, Mineral_Id,Version)
            VALUES ('$mine_id','$month','$year','$i','$condition[$i]','$quantity[$i]','$dressing[$i]','$sell[$i]','$balance[$i]','$Mineral_Ids','$varversion')";
    ...
    if(($minerals[$i] != "")){   /* not allowing empty values and the row which has been removed. */
        ...
        // echo "<script>alert(".$Mineral_Id1.")</script>";
            ...
            // $row1 = mysqli_fetch_assoc($result5);
                $sql4="INSERT INTO details_sale (Mine_Id, Month, Year, no, Ds_export, Ds_royalty, Ds_receipt, Ds_Industry, Ds_buyer, Ds_domestic, Ds_Property, Mineral_Id,Version)
            VALUES ('$mine_id','$month','$year','$i','$export[$i]','$royalty[$i]','$receipt[$i]','$industry[$i]','$buyer[$i]','$domestic[$i]','$Property[$i]','$Mineral_Id1','$varversion')";
...
if(($mine_id != "")){
$sql7="INSERT INTO report (Mine_Id, Month, Year,Version)
VALUES ('$mine_id','$month','$year','$varversion')";
